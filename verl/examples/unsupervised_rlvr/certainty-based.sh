...
export REWARD_TYPE=token_level_entropy
...
export EXPERIMENT_NAME=grpo_${TASK}_certainty-T_${TEMPERATURE}-n_${N_RESPONSES}-kl_${USE_KL}-mbs_${MINI_BATCH_SIZE}-${REWARD_TYPE}-$(date +%Y-%m-%d_%H-%M-%S)
...
python3 -m verl.trainer.main_ppo \
    ...
    unsupervised_reward.type=certainty \
